...
| Date | Change |
|------|--------|
| 2026-05-17 | v2.28.5: **Landing page light Carbon redesign** — Light IBM Carbon theme (white/gray alternating sections, #0f62fe accent, weight-300 headings, 0px border radius); proper chart placement with headings; all 4 SVG charts rewritten for light backgrounds; dark footer inverse pattern; CTA blue-60 banner; tokens updated in page.tsx, landingLayout.ts, landingCharts.tsx, landingAnimations.ts |
| 2026-05-17 | v2.28.4: **Landing page dark redesign — single-file consolidation** — 14 component files consolidated into single page.tsx; dark theme (#0a0a0f bg, #eeeeef text, #4f8ff7 accent); full-bleed sections with 1200px inner; charts placed once with headings |
...
| 2026-05-16 | v2.28.3: **Carbon structural overhaul** — Widened max-width 1200→1584px, fixed surface banding (surface-1 for alternate sections), CTA converted to full-width blue banner, secondary buttons changed to charcoal solid (Carbon button-secondary), card elevation standardized (canvas+hairline), extracted landingLayout.ts shared module (useBreakpoint, badgeStyle, tierColor, TRANSITION), motion timing 0.5s→0.3s cubic-bezier, removed 11 duplicated hooks |
| 2026-05-16 | v2.28.2: **Carbon-faithful landing page fixes** — Systematic fix of all IBM Carbon violations across 14 components: display weight 300, sentence-case eyebrows (ink-muted), weight-400 labels/badges, no box-shadows, dark inverse footer, Carbon button spec (14px/400/12px 16px), primary blue only on CTAs/links; all textTransform uppercase removed, letterSpacing unified to 0.16px |
...
| `astra-app/src/app/page.tsx` | Landing page — single-file consolidation with all sections (Nav, Hero, Demo, Advantage, Findings, Stack, Modules, Cloud, Network, Features, BYO, Enterprise, Competition, Outcomes, CTA Footer); design tokens inline |
| `astra-app/src/components/landing/landingStyles.ts` | DELETED — replaced by landingLayout.ts, then design tokens moved inline to page.tsx |
| `astra-app/src/components/landing/landingLayout.ts` | Shared layout constants (MAX_WIDTH, BREAKPOINTS, SPACING, TRANSITION), hooks (useBreakpoint), and helpers (badgeStyle, tierColor, severityColor) |
